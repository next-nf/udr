...
-moduledoc "Accessor module for the `auth_subscription` aggregate.\n"
           ...
           "  `{ok, NewDoc}`. The caller computes `start = new_sqn - N` from the\n"
           "  doc returned by `update/3`: `maps:get(<<\"sqn\">>, NewDoc) - N`.\n"
...
-doc "Returns a Fun suitable for `update/3` that increments `<<\"sqn\">>` by N.\n"
     ...
     "derives the reserved block start as `maps:get(<<\"sqn\">>, NewDoc) - N`.".
...
upgrade(Doc) ->
    ...
    %% apply defaults. The stamped schema_version will be written back on the
    %% next update/3 call.
    Doc.
