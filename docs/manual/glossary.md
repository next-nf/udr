...
| **Diameter** | The Authentication, Authorization, and Accounting signalling protocol (IETF RFC 6733) used by the EPC. The S6a interface between the MME and the HSS runs over Diameter. |
...
| **S6a** | The Diameter interface between the MME and the HSS in the EPC, carrying AIR/AIA, ULR/ULA, PUR/PUA, and CLR/CLA. It is the primary signalling interface this system serves. |
...
| **`syn`** | An Erlang/OTP process registry and process-group library. This system uses `syn` to coordinate per-IMSI session locking across cluster nodes so that concurrent signalling for one subscriber serializes correctly. |
